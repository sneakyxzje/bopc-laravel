...
@section('title', 'Tải lên Banner')
@section('page_title', 'Tải lên Banner mới')

@section('content')
<div class="max-w-4xl mx-auto" x-data="bannerUpload()">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-black text-near-black tracking-tight">Thêm Mới Banner</h2>
    ...
    <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        @csrf

        <div class="lg:col-span-2">
            <div class="bg-white border border-[#f2f2f2] rounded-md shadow-sm p-6 mb-6">
                <label class="block text-sm font-black text-near-black mb-4 uppercase">Hình ảnh Banner <span class="text-red-500">*</span></label>
                ...
                <div class="relative border-2 border-dashed border-[#ebebeb] rounded-lg bg-[#f7f7f7] hover:bg-[#f2f2f2] transition-colors cursor-pointer overflow-hidden flex flex-col items-center justify-center p-6"
                    ...
                    <input type="file" name="image_path" x-ref="fileInput" @change="previewImage" required accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10 hidden">

                    <template x-if="!previewUrl">
                        <div class="text-center pointer-events-none">
                            <svg class="mx-auto h-12 w-12 text-secondary-gray/30 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                            <p class="text-sm font-bold text-primary">Click để chọn ảnh hoặc kéo thả vào đây</p>
                ...
                @error('image_path')
                <p class="mt-2 text-sm text-red-500 font-medium flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white border border-[#f2f2f2] rounded-md shadow-sm p-6">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <select name="type" required class="w-full px-4 py-3 bg-[#f7f7f7] border border-[#ebebeb] rounded-md focus:bg-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm font-bold text-near-black transition-colors">
                            <option value="slider">Banner Chính</option>
                            <option value="static">Banner Phụ </option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="url" name="link" placeholder="https://..." class="w-full px-4 py-3 bg-[#f7f7f7] border border-[#ebebeb] rounded-md focus:bg-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm font-bold text-near-black transition-colors">
                        <p class="text-[10px] text-secondary-gray/50 mt-1 italic">Khách sẽ được chuyển hướng tới trang này khi click</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="order" value="0" min="0" class="w-full px-4 py-3 bg-[#f7f7f7] border border-[#ebebeb] rounded-md focus:bg-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm font-bold text-near-black transition-colors">
                        <p class="text-[10px] text-secondary-gray/50 mt-1 italic">Số nhỏ hơn sẽ hiển thị trước (0, 1, 2...)</p>
                ...
                <div class="pt-8">
                    <button type="submit" class="w-full py-3.5 bg-primary text-white text-sm font-black uppercase rounded-md hover:bg-primary-hover transition-colors cursor-pointer active:scale-[0.98]">
                        Lưu
                    </button>
                </div>
            </div>
